...
void bfs(int x, int y)
{
    ...
    {
        return; 
        // return because the start is W (water)
    }
    else
    {
        check[x][y] = true;
        ...
        while (!qt.empty())
        {
            ...
            for (int i = 0; i < 4; i++)
            {
                ...
                else
                {
                    ...
                    {
                        continue; 
                        // skiping because it is W (water)
                    }

                    if (arr[nx][ny] == 'L' && !check[nx][ny]) 
                    // if it is L (land) and not visited
                    {
                        check[nx][ny] = true;
                        cnt[nx][ny] = c + 1;
                        ...
                        if (c + 1 > num) 
                        // if the current time is larger than the saved time
                        {
                            num = c + 1;
                        }
                    }
                    ...
                    {
                        if (cnt[nx][ny] > c + 1) 
                        /* comparing if the visited coordinate has larger time 
                        than the current time (if is larger then 
                        it means that the time save is not the time for 
                        the shortest distance between the treasures) */
                        {
                            cnt[nx][ny] = c + 1;
                            qt.push(make_tuple(nx, ny, c + 1));
